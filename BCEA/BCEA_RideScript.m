...
for i = 1:length(fileList)
    ...
    if ismember(fullFileName, specificFiles)
        ...
        data.(fileNameWithoutExt).x_ride=data_ride.(fileNameWithoutExt).x_filt;
        data.(fileNameWithoutExt).y_ride=data_ride.(fileNameWithoutExt).y_filt;
...
for i=1:length(fileList)
    ...
    fileNameWithoutExt = matlab.lang.makeValidName(fileNameWithoutExt);
    if ismember(fullFileName, specificFiles)
        x = data.(fileNameWithoutExt).x_ride;  
        y = data.(fileNameWithoutExt).y_ride;
        k=3;
        data.(fileNameWithoutExt).sigma_x_ride=std(x);
        data.(fileNameWithoutExt).sigma_y_ride=std(y);
        data.(fileNameWithoutExt).mean_x_ride=mean(x);
        data.(fileNameWithoutExt).mean_y_ride=mean(y);
        data.(fileNameWithoutExt).rho_ride=corr(x,y);
        data.(fileNameWithoutExt).BCEA_ride=2*pi*k*data.(fileNameWithoutExt).sigma_x_ride*data.(fileNameWithoutExt).sigma_y_ride*sqrt(1-data.(fileNameWithoutExt).rho_ride^2);
        ...
        data.(fileNameWithoutExt).ellipse_x = data.(fileNameWithoutExt).ellipse_points(1,:) + data.(fileNameWithoutExt).mean_x_ride;
        data.(fileNameWithoutExt).ellipse_y = data.(fileNameWithoutExt).ellipse_points(2,:) + data.(fileNameWithoutExt).mean_y_ride;
        ...
        x_guzik = data.(fileNameWithoutExt).x_ride - data.(fileNameWithoutExt).mean_x_ride;
        y_guzik = data.(fileNameWithoutExt).y_ride - data.(fileNameWithoutExt).mean_y_ride;
        xy = [x_guzik(:), y_guzik(:)]'; 
...
for i=1:length(fileList)
    ...
    if ismember(fullFileName, specificFiles)
        ...
        data.(fileNameWithoutExt).AppEntX=approximateEntropy(data.(fileNameWithoutExt).x_ride);
        data.(fileNameWithoutExt).SampEntY_all = sampen_vals_y;
        data.(fileNameWithoutExt).SampEntY_mean = mean(sampen_vals_y, 'omitnan'); 
        data.(fileNameWithoutExt).AppEntY=approximateEntropy(data.(fileNameWithoutExt).y_ride);
...
for i = 1:length(fileList)
    ...
    if ismember(fullFileName, specificFiles) 
        ...
        subplot(1, 3, 1);
        scatter(data.(fileNameWithoutExt).x_ride, data.(fileNameWithoutExt).y_ride, 'k.');
        grid on;
        ...
        hold on;
        grid on;
        mu_x=data.(fileNameWithoutExt).mean_x_ride;
        mu_y=data.(fileNameWithoutExt).mean_y_ride;
...
%         end
        ...
        %[counts, centers] = hist3([data.(fileNameWithoutExt).x_ride, data.(fileNameWithoutExt).y_ride], 'Nbins', [50, 50]);
        %scale = 6;
...
%         ylabel('Y decentration (min arc)');
    ...
    jpg_filename = sprintf('%sBCEARide.jpg', (fileNameWithoutExt));
    eps_filename = sprintf('%sBCEARide.eps', (fileNameWithoutExt));
...
for i = 1:length(fileList)
    ...
    if ismember(fullFileName, specificFiles) 
        ...
        fh = figure(i);
        fh.WindowState = 'maximized';
        data_density = [data.(fileNameWithoutExt).x_ride, data.(fileNameWithoutExt).y_ride];

        [density, ~] = ksdensity(data_density, data_density, 'Bandwidth', []);

        densityNorm = (density - min(density)) / (max(density) - min(density));
        scatter(data.(fileNameWithoutExt).x_ride, data.(fileNameWithoutExt).y_ride, 20, densityNorm, 'filled');
        hold on
        grid on
        colormap(jet);
        colorbar;
    ...
    jpg_filename = sprintf('%sBCEARideColor.jpg', (fileNameWithoutExt));
    eps_filename = sprintf('%sBCEARideColor.eps', (fileNameWithoutExt));
    saveas(gcf, jpg_filename, 'jpg');
    saveas(gcf, eps_filename, 'epsc');
...
for i = 1:length(fileList)
    ...
    jpg_filename = sprintf('%sBCEARideColorCool.jpg', (fileNameWithoutExt));
    eps_filename = sprintf('%sBCEARideColorCool.eps', (fileNameWithoutExt));
    saveas(gcf, jpg_filename, 'jpg');
    saveas(gcf, eps_filename, 'epsc');
...
for i = 1:length(fileNames) 
    ...
    if ismember(fullFileName, specificFiles) 
        ...
        STD_x=data.(fileNameWithoutExt).sigma_x_ride;
        STD_y=data.(fileNameWithoutExt).sigma_y_ride;
        MEAN_x=data.(fileNameWithoutExt).mean_x_ride;
        MEAN_y=data.(fileNameWithoutExt).mean_y_ride;
        RHO=data.(fileNameWithoutExt).rho_ride;
        BCEA=data.(fileNameWithoutExt).BCEA_ride;
        ...
        BCEAmapRide= table(STD_x,STD_y,MEAN_x,MEAN_y,RHO,BCEA,GuziksIndex,MSE,SampEntX_mean,AppEntX,SampEntY_mean,AppEntY,NumPRL);
        BCEAcsvFilePath = [(fileNameWithoutExt),'_BCEARide', '.csv'];
        writetable(BCEAmapRide,BCEAcsvFilePath,'Delimiter',',');
...
featurescsvFilePath = ['BCEA_RideValues', '.csv'];
